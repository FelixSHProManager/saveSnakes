...
/** 新故事默认归入的子系列（对应 系列归类.md 中 # 待定 / ## 待定） */
export const PENDING_SUB = '待定';
...
/** 将 archive 中尚未归类的新故事写入 _series_map.json，默认归入「待定」 */
export function syncPendingStories() {
  const allSubSeries = getAllSubSeries();
  if (!allSubSeries.has(PENDING_SUB)) {
    throw new Error('系列归类.md 中缺少「待定」子系列，请先添加 # 待定 / ## 待定');
  }

  const map = fs.existsSync(MAP_FILE) ? loadSeriesMap() : {};
  const titles = listArchiveTitles();
  const added = [];

  for (const title of titles) {
    if (!map[title]) {
      map[title] = PENDING_SUB;
      added.push(title);
    }
  }
  ...
  return { added, storyCount: titles.length };
}
